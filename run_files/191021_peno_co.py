...
test_set_size = 1000
noise_scale = 3
beta = np.array([1.2, 1, 0.8, 0.6, 0.4])
...
regularizers = np.array(["peno"])
regularizer_names = np.array(["Penalizing wrong order"])
regularizer_labels = np.array(["Penalizing wrong order"])
...
for lr_ix, lr in enumerate(learning_rates):
    ...
    for reg_ix, regularizer in enumerate(regularizers):  # reg_ix = 0; regularizer = "peno"
